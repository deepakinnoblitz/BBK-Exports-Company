...
def get_monthly_roster(month, year, department=None, employee=None):
	...
	Each cell contains the resolved line, source, and day attributes.
	"""
	m = cint(month)
	y = cint(year)
	if not m or not y:
	...
	# 3. Pre-fetch all active Line Order records for lookup
	...
	# 4. Fetch all active line roster assignments for the entire month
	...
	for emp in employees_data:
		...
		for day_info in days_list:
			...
			if assigned_roster:
				...
				resolved = get_applicable_line(e_id, d_str)
				if resolved:
					l_name = resolved["line_order"]
					l_meta = line_lookup.get(l_name, {})
					emp_lines[d_str] = {
						"line_order": l_name,
						"line_name": resolved.get("line_name", l_name),
